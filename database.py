...
class DatabaseManager:
    ...
    async def create_tables(self):
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute(
                    "CREATE UNIQUE INDEX IF NOT EXISTS ux_transactions_reference "
            ...
            async with conn.transaction():
                ...
                row = await conn.fetchrow("""
                    INSERT INTO users(
                        ...
                        balance, total_deposits, referral_code, referred_by
                    )
    ...
    async def _mutate_balance(
        ...
        await conn.execute(
            "UPDATE users SET balance=$1,last_seen=NOW() WHERE id=$2",
            new,
            user_id,
        )

        if delta > 0 and transaction_type in {
            "deposit", "welcome_bonus", "admin_deposit"
        }:
            await conn.execute(
        ...
        if delta > 0 and transaction_type == "win":
            await conn.execute(
                ...
                "games_won=games_won+1 WHERE id=$2",
                delta,
                user_id,
            )
        ...
        if delta < 0 and transaction_type in {
            ...
            await conn.execute(
                "UPDATE users SET total_withdrawals=total_withdrawals+$1 "
                "WHERE id=$2",
                -delta,
                user_id,
            )
        ...
        await conn.execute("""
            INSERT INTO transactions(
                user_id,type,amount,balance_after,status,reference,description
            )
    ...
    async def update_balance(
        ...
        reference = (
            reference
            or f"{transaction_type}:{user_id}:{datetime.utcnow().timestamp()}"
        )
    ...
    async def reserve_bet_by_telegram_id(
        self, telegram_id: str, amount, reference: str, description: str
    ) -> bool:
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                user = await conn.fetchrow(
                ...
                return await self._mutate_balance(
                    conn, user["id"], -money(amount), "bet",
                    description, reference
                )

    async def refund_bet_by_telegram_id(
        self, telegram_id: str, amount, reference: str, description: str
    ) -> bool:
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                return await self._mutate_balance(
                    conn, user["id"], money(amount), "refund",
                    description, reference + ":refund"
                )

    async def pay_winner_once_by_telegram_id(
        self, telegram_id: str, amount, reference: str, description: str
    ) -> bool:
        async with self.pool.acquire() as conn:
            async with conn.transaction():
    ...
    async def create_game_record(
        ...
        async with self.pool.acquire() as conn:
            await conn.execute("""
                INSERT INTO games(
                    game_id,room_id,status,card_price,prize_percentage,
                    min_players,max_players,server_seed,game_hash
                )
                VALUES($1,$2,'waiting',$3,$4,$5,$6,$7,$8)
                ON CONFLICT(game_id) DO NOTHING
            """, game_id, room_id, card_price, prize_percentage,
                min_players, max_players, server_seed, game_hash)

    async def update_game_financials(
        self, game_id, total_bet, prize_pool, commission
    ):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE games SET total_bet=$1,prize_pool=$2,"
                "commission=$3 WHERE game_id=$4",
                total_bet, prize_pool, commission, game_id,
            )

    async def update_game_called_numbers(self, game_id, numbers):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE games SET called_numbers=$1 WHERE game_id=$2",
                json.dumps(numbers),
                game_id,
            )
    ...
    async def update_game_status(self, game_id, status, timestamp=None):
        async with self.pool.acquire() as conn:
            if status == "active":
                await conn.execute(
                    "UPDATE games SET status='active',started_at=$1 "
                    "WHERE game_id=$2",
                ...
                await conn.execute(
                    "UPDATE games SET status='finished',finished_at=$1 "
                    "WHERE game_id=$2",
                    timestamp or datetime.utcnow(),
                    game_id,
                )
        ...
        async with self.pool.acquire() as conn:
            await conn.execute("""
                UPDATE games SET
                    ...
                    winners=$1,
                    finished_at=$2,
                    server_seed=COALESCE($3,server_seed),
                    game_hash=COALESCE($4,game_hash),
                    called_numbers=COALESCE($5,called_numbers)
                WHERE game_id=$6
            """, json.dumps(winners), finished_at, server_seed, game_hash,
                json.dumps(called_numbers) if called_numbers is not None else None,
                game_id)

            for uid in winners:
                amount = money(
                    (winner_amounts or {}).get(str(uid), 0)
                )
                await conn.execute("""
                    UPDATE game_players
                    SET is_winner=TRUE,
                    ...
                    WHERE game_id=(SELECT id FROM games WHERE game_id=$2)
                      AND user_id=(
                          SELECT id FROM users WHERE telegram_id=$3
                      )
                """, amount, game_id, str(uid))

    async def add_player_to_game_by_telegram_id(
        self, game_id, telegram_id, card_number, card_data
    ):
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                game = await conn.fetchrow(
                    "SELECT id FROM games WHERE game_id=$1 FOR UPDATE",
                    game_id,
                )
                user = await conn.fetchrow(
                    "SELECT id FROM users WHERE telegram_id=$1 FOR UPDATE",
                ...
                inserted = await conn.fetchval("""
                    INSERT INTO game_players(
                        game_id,user_id,card_number,card_data,marked_numbers
                    )
                    VALUES($1,$2,$3,$4,$5)
                    ON CONFLICT(game_id,user_id) DO NOTHING
                    RETURNING id
                """, game["id"], user["id"], str(card_number),
                    json.dumps(card_data),
                    json.dumps([card_data[12]]))
    ...
    async def create_deposit_request(
        self, telegram_id, amount, method, external_reference
    ):
        async with self.pool.acquire() as conn:
            user = await conn.fetchrow(
            ...
            try:
                row = await conn.fetchrow("""
                    INSERT INTO deposit_requests(
                        user_id,amount,payment_method,external_reference
                    )
                    VALUES($1,$2,$3,$4)
                    RETURNING id,status
    ...
    async def create_withdrawal_request(
        self, telegram_id, amount, method, details
    ):
        amount = money(amount)
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                if not await self._mutate_balance(
                    conn,
                    user["id"],
                    -amount,
                    "withdrawal_pending",
                ...
                row = await conn.fetchrow("""
                    INSERT INTO withdrawal_requests(
                        user_id,amount,payment_method,payment_details,reference
                    )
                    VALUES($1,$2,$3,$4,$5)
                    RETURNING id,status
    ...
    async def approve_deposit(
        self, request_id: int, admin_user_id: int
    ) -> bool:
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                reference = (
                    f"deposit:{req['payment_method']}:"
                    f"{req['external_reference']}"
                )

                ok = await self._mutate_balance(
                    conn,
                    req["user_id"],
                    money(req["amount"]),
                    "deposit",
                ...
                await conn.execute(
                    "UPDATE deposit_requests SET status='approved',"
    ...
    async def approve_withdrawal(
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute(
                    "UPDATE users SET total_withdrawals="
                    "total_withdrawals+$1 WHERE id=$2",
                    money(req["amount"]),
                    req["user_id"],
                )
    ...
    async def set_user_banned(
        self, telegram_id: str, banned: bool, admin_user_id: int
    ):
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                user = await conn.fetchrow(
    ...
    async def get_leaderboard(self, days=30, limit=10):
        async with self.pool.acquire() as conn:
            rows = await conn.fetch("""
                SELECT username,first_name,games_won,balance,total_wins,
                       games_played
                FROM users
                WHERE games_played>0
                ORDER BY games_won DESC,total_wins DESC
                LIMIT $1
            """, limit)
